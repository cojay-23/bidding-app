...
API_KEY = os.environ.get("MOONSHOT_API_KEY") or os.environ.get("DEEPSEEK_API_KEY") or ""
...
输出用倒计时时间轴，从当前日期（2026-06-21）开始计算剩余天数。
...
提取：正本/副本份数、装订方式（胶装/线装/骑缝钉）、封装要求（密封章位置、封条格式）、电子版介质要求（U盘/光盘/光盘+U盘）、封套上的标识信息、签字盖章要求（哪些地方盖公章、法人章、骑缝章）。
...
输出一个完整的 HTML 文档（从 <!doctype html> 开始），包含以下 12 章：
...
def call_llm(prompt: str) -> str:
    ...
    # kimi-k2 系列模型只支持 temperature=1，其他模型用 0.3
    temperature = 1 if MODEL.startswith("kimi-") else 0.3
